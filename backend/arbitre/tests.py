...
REALM_NAME = "arbitre"
CLIENT_ID = "arbitre-frontend"

KEYCLOAK = keycloak.KeycloakOpenID(
...
# Use this to return the basic testing user
def get_testing_user():
    return User.objects.get(email="testinguser@test.com")


def get_testing_user_username():
    return User.objects.get(email="testinguser@test.com").username
...
class UserTest(TestCase):
        ...
        KEYCLOAK_ADMIN.create_user(
            {
                ...
                "lastName": "User",
                "enabled": True,
                "credentials": [{"value": "testing_user", "type": "password"}],
            },
            exist_ok=True,
        )

        # make request to put user in django users db
        token = KEYCLOAK.token("testing_user", "testing_user")
        access_token = token["access_token"]
        endpoint = cls.BASE_URL + "/api/course/"
    ...
    def test_login_user_fail_wrong_password(self):
        with self.assertRaises(keycloak.exceptions.KeycloakAuthenticationError):
            KEYCLOAK.token("testing_user", "testing_user2")
