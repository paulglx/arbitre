...
} from "../../features/courses/sessionApiSlice";
...
const Session = () => {
  ...
  const isOwner = session?.course?.owners
    ?.map((o: any) => o.username)
    .includes(username);
  const isTutor = session?.course?.tutors
  ...
  const handleUpdate = async () => {
    ...
    const startDateFormatted =
      startDate === ""
        ? null
        : moment(startDate).format("YYYY-MM-DDTHH:mm:ss.SSSSSSZ");
    const deadlineFormatted =
      deadline === ""
        ? null
        : moment(deadline).format("YYYY-MM-DDTHH:mm:ss.SSSSSSZ");
    ...
    return isOwner || isTutor ? (
      <>
        ...
        {actionsDropdown && (
          <div
            id="dropdown"
            className="absolute mt-14 ml-14 border z-10 bg-white divide-y divide-gray-100 rounded-lg shadow"
          >
            <ul
  ...
  if (sessionIsError) {
    return (
      <div className="d-flex align-items-center justify-content-center vh-100 bg-light">
        <h3>
          The session you are looking for doesn't exist, <br />
          or you aren't allowed to access it.
          <br />
