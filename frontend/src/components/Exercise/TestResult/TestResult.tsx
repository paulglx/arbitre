...
} from "../../../features/submission/submissionApiSlice";
...
const TestResult = (props: any) => {
  ...
  const accessToken = useSelector(selectCurrentKeycloakToken);

  const { data: testResults, isLoading: testResultsIsLoading } =
    useGetSubmissionTestResultsQuery(
  ...
  useEffect(() => {
    const eventSource = new EventSource(
      `${process.env.REACT_APP_API_URL}/events/submission/${exercise_id}?authorization=${accessToken}`
    );
  ...
  useEffect(() => {
    ...
    if (
      submissionData[0]?.status === "pending" ||
      submissionData[0]?.status === "running" ||
      testResults?.some(
        (result: any) =>
    ...
    if (result.running) {
      ...
      return (
        <span className="font-monospace flex">
          ...
          &nbsp;
          <pre className="w-full overflow-y-scroll">{result.stdout}</pre>
        </span>
      );
    }
  };
    ...
    return (
      <>
        <p className="text-danger">
          There was an error while trying to display the test results.
        </p>
