...
const EditableName = (props: any) => {
    ...
    const handleSetStudentGroupName = async () => {
        ...
        await changeStudentGroupName({
            ...
            .unwrap()
            .catch((e) => {
                dispatch(pushNotification({ message: "Unable to change student group name.", type: "error" }))
            })
            .then((res) => {
                if (res.course) {
                    setCourse(res.course)
                    dispatch(pushNotification({ message: "Group name changed successfully", type: "success" }))
                }
            })
    }
